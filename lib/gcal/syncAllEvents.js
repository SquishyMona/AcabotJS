...
export const syncAllEvents = async (calendarId, scheduledEventManager, sendDiscordToGCal) => {
    ...
    for await (const event of events) {
        let googleStart = event.start.dateTime ? new Date(event.start.dateTime).toISOString() : new Date(event.start.date).toISOString();
        let googleEnd = event.start.dateTime ? new Date(event.end.dateTime).toISOString() : new Date(event.end.date).toISOString();
        
        const scheduledEvent = scheduledEvents.find((scheduledEvent) => {
            if (event.recurrence && new Date(googleStart).getTime() < new Date().getTime()) {
                console.log(`Event has already occurred, comparing next occurrence`);
                const dtstart = googleStart.replaceAll('-', '').replaceAll('.', '').replaceAll(':', '').slice(0, -4);
                const nextOccurrence = getNextOccurrence(`DTSTART:${dtstart};\n${event.recurrence[0].split(':')[1]}`, new Date());    
                googleStart = nextOccurrence.toISOString();
                googleEnd = new Date(new Date(googleStart).getTime() + new Date(event.end.dateTime ?? event.end.date).getTime() - new Date(event.start.dateTime ?? event.start.date).getTime()).toISOString();
            }
        ...
        if (!scheduledEvent) {
            ...
            if (event.recurrence) {
                newRecurrenceRule = parseRecurrenceRule(event.recurrence[0].split(':')[1], googleStart);
            }
...
const getNextOccurrence = (recurrenceRule, startAt) => {
    ...
    return rule.after(new Date(startAt));
}
...
const parseRecurrenceRule = (recurrenceRule, googleStart) => {
    ...
    console.log(`Parsed Recurrence Rule: ${JSON.stringify(newRecurrenceRule)}`);   
