...
export const syncAllEvents = async (calendarId, scheduledEventManager, sendDiscordToGCal) => {
    ...
    for await (const event of events) {
        let googleStart = event.start.dateTime ? new Date(event.start.dateTime).toISOString() : new Date(`${event.start.date}T00:00:00-05:00`).toISOString();
        let googleEnd = event.start.dateTime ? new Date(event.end.dateTime).toISOString() : new Date(`${event.end.date}T00:00:00-05:00`).toISOString();
        let newRecurrenceRule;
        if (event.recurrence) {
            newRecurrenceRule = parseRecurrenceRule(event.recurrence[0].split(':')[1], googleStart, calendar, calendarId, event.id);
        }
        if (newRecurrenceRule === false) {
            continue;
        } else {
            googleStart = newRecurrenceRule.startAt;
            googleEnd = new Date(new Date(googleStart).getTime() + new Date(googleEnd).getTime() - new Date(googleStart).getTime()).toISOString();
        }
