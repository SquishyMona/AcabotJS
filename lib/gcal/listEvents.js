...
export async function listEvents(userId, calendarId, startDate=null, endDate=null) {
	...
	startDate ? requestPayload.timeMin = new Date(startDate) : new Date();
	endDate ? requestPayload.timeMax = new Date(endDate) : new Date(requestPayload.timeMin.getTime() / (1000 * 60 * 60 * 24));
